...
Fecha a tarefa no ADVBOX:
  ...
  - From = AGENTE_IA_ID (retorno aparece em nome da propria PASCOAL.IA)
  - Em caso de sucesso, grava um marcador maquina-legivel ([PASCOALIA-FEITO #id])
    que permite a PASCOAL.IA detectar, em varreduras futuras, que a tarefa ja foi
...
from crm import enviar_texto_por_telefone  # noqa
...
def _marcador_feito(task_id) -> str:
    """Marcador embutido no comentario de retorno (ex.: '[PASCOALIA-FEITO #12345]')."""
...
def ja_concluida_no_advbox(task_id, lawsuit_id) -> bool:
    ...
    Antes de (re)executar uma tarefa, a PASCOAL.IA varre os andamentos/publicacoes do
    processo. Se encontrar o marcador de conclusao que ela mesma deixou referenciando
    esta tarefa original (ex.: "[PASCOALIA-FEITO #12345]"), retorna True -> nao refazer.
    ...
    for p in posts:
        if alvo.search(_texto_post(p)):
            log.info(f'[{task_id}] marcador de conclusao encontrado no processo '
                     f'{lawsuit_id} - tarefa ja feita pela PASCOAL.IA, nao sera refeita')
            return True
    return False
...
def entregar_resultado(task_id, destinatarios, lawsuit_id, resultado: dict):
    ...
    if sucesso:
        # Marcador maquina-legivel: assinala que ESTA tarefa ja foi feita pela PASCOAL.IA.
    ...
    try:
        criar_publicacao(
            ...
            comments=comments,
            from_id=AGENTE_IA_ID,     # retorno do agente aparece como da propria PASCOAL.IA
