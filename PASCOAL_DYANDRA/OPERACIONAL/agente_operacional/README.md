# Agente Operacional PASCOAL (PASCOAL.IA)

Agente autonomo que recebe tarefas do ADVBOX (atribuidas ao usuario-agente **PASCOAL.IA**) via webhook disparado pela N8N, produz o resultado (peca juridica, notificacao de cliente, resposta operacional) e devolve a tarefa para quem pediu com o link do documento.
...
Equipe do escritorio
   |
   v  (atribui tarefa a PASCOAL.IA no ADVBOX)
...
AGENTE_OP_PORT=8787
ADVBOX_USER_AGENTE=<id do usuario-agente PASCOAL.IA no ADVBOX>
ADVBOX_TASK_TYPE_ACOMPANHAMENTO=<id do tipo de tarefa para retornos>
ESCRITORIO_ASSINATURA=Alexandre Pascoal Marques - OAB/SP 270.924\nFlávia Dyandra da Silva - OAB/SP 352.455\nVitória Freitas - OAB/SP 508.842
...
- **So PASCOAL.IA pendente** -> trocar `SEU_ADVBOX_USER_AGENTE` pelo ID do usuario-agente no ADVBOX
...
- Retorno no ADVBOX sempre com `from = usuario-agente (PASCOAL.IA)`
