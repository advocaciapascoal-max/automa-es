...
N8N monitora tarefas ADVBOX atribuidas ao usuario-agente (PASCOAL.IA) e dispara:
...
Tambem expoe o receptor de eventos do CRM de atendimento (Leadone/GoHighLevel),
montado a partir de webhook_crm.py:
  POST /crm/webhook   mensagem recebida / contato criado no CRM
  GET  /crm/status    diagnostico da integracao
...
from .webhook_crm import router as crm_router
...
app = FastAPI(title='Agente Operacional PASCOAL', version='1.0')
...
@app.get('/healthcheck')
def healthcheck():
    return {'status': 'ok', 'servico': 'agente_operacional_pascoal'}
