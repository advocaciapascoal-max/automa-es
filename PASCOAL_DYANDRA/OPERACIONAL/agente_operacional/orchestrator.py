...
def _agente_pendente(tarefa):
    """True se o agente (PASCOAL.IA) esta nos destinatarios e ainda nao completou."""
...
def processar_tarefa(task_id: int, tarefa: dict = None):
    ...
    try:
        ...
        if not _agente_pendente(tarefa):
            log.info(f'[{task_id}] Ignorada: PASCOAL.IA nao e destinataria pendente')
        ...
        if ja_concluida_no_advbox(task_id, lawsuit_id):
            log.info(f'[{task_id}] Ja concluida pela PASCOAL.IA no ADVBOX - ignorando (nao refaz)')
