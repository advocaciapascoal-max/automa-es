...
def enviar_para_zapsign(drive_service, nome_cliente, docs_gerados, dados_cliente):
    ...
    O segundo signatario do contrato (advogado responsavel) vem do .env:
      ADVOGADO_RESPONSAVEL_NOME / ADVOGADO_RESPONSAVEL_EMAIL
    """
    nome = dados_cliente.get('nome', nome_cliente)
    email = dados_cliente.get('email', '')
    telefone = dados_cliente.get('telefone', '')

    adv_nome = os.getenv('ADVOGADO_RESPONSAVEL_NOME', 'Alexandre Pascoal Marques')
    adv_email = os.getenv('ADVOGADO_RESPONSAVEL_EMAIL', '')
    ...
    for doc in docs_gerados:
        ...
        if is_contrato:
            signatarios.append(criar_signatario(adv_nome, adv_email or None))
